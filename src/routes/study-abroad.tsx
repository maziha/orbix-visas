import { createFileRoute, Link, Outlet, useMatches } from "@tanstack/react-router";
import { SiteLayout } from "@/components/site/SiteLayout";
import { CountryFlag } from "@/components/site/CountryFlag";
import { ConsultationCta, SectionEyebrow, SectionHeading } from "@/components/site/HomeSections";
import { COUNTRIES } from "@/lib/countries";
import { PAGE_DESCRIPTIONS } from "@/lib/page-descriptions";
import { PAGE_TITLES } from "@/lib/page-titles";

export const Route = createFileRoute("/study-abroad")({
  head: () => ({
    meta: [
      { title: PAGE_TITLES.studyAbroad },
      { name: "description", content: PAGE_DESCRIPTIONS.studyAbroad },
    ],
  }),
  component: Layout,
});

function Layout() {
  const matches = useMatches();
  const isChild = matches.some((m) => m.routeId.startsWith("/study-abroad/"));
  if (isChild) {
    return (
      <SiteLayout>
        <Outlet />
      </SiteLayout>
    );
  }
  return (
    <SiteLayout>
      <section className="py-20 bg-brand-dark text-white">
        <div className="container-px mx-auto max-w-7xl">
          <SectionEyebrow tone="dark">STUDY ABROAD</SectionEyebrow>
          <h1 className="font-display text-4xl md:text-6xl mt-0">Choose Your Destination</h1>
          <p className="text-white/80 mt-4 max-w-2xl text-lg">
            Explore study pathways across 7 countries — with guidance tailored to your goals and budget.
          </p>
        </div>
      </section>
      <section className="py-20">
        <div className="container-px mx-auto max-w-7xl">
          <SectionHeading
            eyebrow="EXPLORE DESTINATIONS"
            title="Study Destinations"
            align="left"
            className="max-w-none mb-10"
          />
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {COUNTRIES.map((c) => (
              <div key={c.slug} className="card-lift bg-brand-white rounded-xl p-8 flex flex-col">
                <CountryFlag code={c.code} size="lg" title={c.name} className="mb-3" />
                <h3 className="font-display text-2xl text-[var(--navy)]">{c.name}</h3>
                <Link
                  to="/study-abroad/$country"
                  params={{ country: c.slug }}
                  className="btn-secondary mt-4 self-start"
                >
                  Explore →
                </Link>
              </div>
            ))}
          </div>
        </div>
      </section>
      <ConsultationCta
        title="Need Help Choosing a Country?"
        subtitle="Tell us about your academic background and goals — we'll suggest destinations and next steps."
      />
    </SiteLayout>
  );
}
